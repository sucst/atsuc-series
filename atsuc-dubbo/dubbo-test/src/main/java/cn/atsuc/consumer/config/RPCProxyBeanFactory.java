package cn.atsuc.consumer.config;
...
public class RPCProxyBeanFactory implements FactoryBean<IUserService>, Runnable {
    ...
    public RPCProxyBeanFactory() throws InterruptedException {
        ...
        while (null == channel) {
            Thread.sleep(550);
            log.info("Rpc Socket 链接等待，需要启动 dubbo 默认提供 127.0.0.1:20881 链接地址...");
        }
    }
    ...
    public void run() {
        ...
        try {
            ...
            ChannelFuture channelFuture = bootstrap.connect("127.0.0.1", 20881).syncUninterruptibly();
